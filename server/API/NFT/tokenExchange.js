    ...
    if (userTokenAmount < amount){
        ...
        try{				
            const getGasAmountForExchange=  () => {
                ...
                const gasAmount = contract.methods.transfer(address, amount).estimateGas({ from: serverAddress })
                return gasAmount
            }
            ...
            let contract20 = new web3.eth.Contract(contract20ABI, contract20Address, {from: serverAddress} ); 
            ...
            const signedTx = await web3.eth.accounts.signTransaction(rawTransaction20, privateKey)
            web3.eth.sendSignedTransaction(signedTx.rawTransaction) 
            .then(req => { 
                User.decrement({haes_sal_amount:amount},{where:{address:address}}); 
                User.increment({ip_amount:amount},{where:{address:address}}); 
                console.log('교환 선공')
