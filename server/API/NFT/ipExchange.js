...
const ipExchange = async (req, res) => {
    const data =req.body;
    ...
    const userTokenAmount = callUserData.dataValues.token_amount;
    ...
    if (userTokenAmount < amount){
		...
		return res.status(400).send('IP가 부족합니다.')
	} else {
        try{				
            const getGasAmountForExchange=  () => {
                const contract = new web3.eth.Contract(contract20ABI, contract20Address);
                const gasAmount = contract.methods.transfer(serverAddress, amount).estimateGas({ from: address })
                return gasAmount
            }
            ...
            let contract20 = new web3.eth.Contract(contract20ABI, contract20Address, {from: address} ); 
            let data20 = contract20.methods.transfer(serverAddress, amount).encodeABI(); //(recipient, tokenuri, 가격)
            let rawTransaction20 = {"to": contract20Address, "gas": gasforExchange, "data": data20 }; 
            const signedTx = await web3.eth.accounts.signTransaction(rawTransaction20, userPrivateKey)
            web3.eth.sendSignedTransaction(signedTx.rawTransaction) 
            .then(req => { 
                User.decrement({ip_amount:amount},{where:{address:address}}); 
                User.increment({haes_sal_amount:amount},{where:{address:address}}); 
                console.log('교환 선공')
