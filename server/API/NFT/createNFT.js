...
async function run() {
  // const makeDir = await ipfs.files.mkdir(`/${req.files[0].filename}`)
  // const stats = await ipfs.files.stat(`/${req.files[0].filename}`)
  // console.log(stats)
}
...
const createNFT = async (req, res) => {
  ...
  if (req.files.length == 1) {
    ...
    await ipfs.files.mkdir("/new");
    for (let i = 0; i < req.files.length; i++) {
      const ipfsStats = async (img) => {
        const file = {
          path: `new`,
          content: img,
        };
        const addFile = await ipfs.add(file);
        return addFile;
      };

      let testFile = fs.readFileSync(`uploads/${req.files[i].filename}`, null);
      let testBuffer = Buffer.from(testFile); //new Buffer -> Buffer.from
      const ipfsInfo = await ipfsStats(testBuffer);
      console.log("ipfsImgurl : ", ipfsInfo);
      console.log("/" + ipfsInfo.path);

      for await (const file of ipfs.files.ls("/new")) {
        console.log(file.name);
      }
    }
    const stats = await ipfs.files.stat("/new");
    console.log(stats);
    const cid = "QmUhEGMXZa8H3ziEqCHNWGLDAJco6HSAXUVWbKCuWt4TpA";
  ...
  //           const gasAmount = contract.methods.approve(contract721Address, 1)
